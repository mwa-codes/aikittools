...
export async function POST(req: NextRequest) {
  try {
    ...
    const { company, role, experience, tone } = body as {
      company?: string;
      role?: string;
      experience?: string;
      tone?: string;
    ...
    if (!company || !role || !experience) {
      return NextResponse.json(
        { error: "Company, role, and relevant experience are required." },
        { status: 400 }
      );
    }
    ...
    if (wordCount > MAX_WORDS) {
      return NextResponse.json(
        { error: `Experience is too long. Please limit to ${MAX_WORDS} words (yours: ${wordCount}).` },
        { status: 400 }
      );
    }
...
Return only the cover letter text, nothing else.`,
    ...
    console.error("[/api/tracker/cover-letter] Error:", err);
